        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              <Link to="/admin" className="text-sm font-medium text-slate-400 hover:text-blue-600 transition-colors">
                Admin
              </Link>
            </nav>
              ...
              <div className="px-4 py-4 space-y-2">
                ...
                <div className="pt-2">
                  <a
                    href="#book"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex w-full items-center justify-center px-5 py-3 text-base font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-full shadow-md shadow-blue-200 mb-2"
                  >
                    Book Now
                  </a>
                  <Link
                    to="/admin"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex w-full items-center justify-center px-5 py-3 text-base font-medium text-slate-600 hover:bg-slate-100 rounded-md"
                  >
                    Admin Login
                  </Link>
                </div>
              ...
              <div className="lg:col-span-3 glass p-8 md:p-10 rounded-3xl">
                ...
                {formStatus === 'submitted' ? (
                  ...
                  >
                    ...
                    <p className="text-slate-600 max-w-sm mx-auto">Thank you. Your request has been securely saved. Our front desk team will contact you shortly.</p>
                  </motion.div>
                ) : formStatus === 'error' ? (
                  <motion.div 
                    initial={{ opacity: 0, scale: 0.95 }}
                  ...
                  >
                    ...
                    <h4 className="text-2xl font-bold text-slate-900 mb-3">Connection Error</h4>
                    <p className="text-slate-600 max-w-sm mx-auto mb-6">There was an issue saving your request to the database. Please try again.</p>
                    <button onClick={() => setFormStatus('idle')} className="px-6 py-2 bg-slate-200 hover:bg-slate-300 rounded-lg font-medium text-slate-800 transition-colors">
                      Try Again
                    </button>
                  </motion.div>
                ) : (
                  <form onSubmit={handleBookSubmit} className="space-y-6">
                    <div className="space-y-1.5">
                      ...
                      <input required type="text" id="name" value={formData.name} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl bg-white/50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm transition-shadow" placeholder="Full Name" />
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div className="space-y-1.5">
                        <label htmlFor="phone" className="text-xs font-bold text-slate-500 uppercase tracking-wide">Phone Number</label>
                        <input required type="tel" id="phone" value={formData.phone} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl bg-white/50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm transition-shadow" placeholder="(555) 000-0000" />
                      </div>
                      <div className="space-y-1.5">
                        <label htmlFor="date" className="text-xs font-bold text-slate-500 uppercase tracking-wide">Preferred Date</label>
                        <input required type="date" id="date" value={formData.date} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl bg-white/50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm transition-shadow text-slate-500" />
                      </div>
                    </div>
                    ...
                    <div className="pt-2">
                      <button disabled={formStatus === 'submitting'} type="submit" className="w-full flex justify-center items-center py-4 rounded-xl shadow-lg shadow-blue-100 text-base font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 transition-all">
